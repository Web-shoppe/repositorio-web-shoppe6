...
SELECT p.*, 
       ...
       l.foto_loja, 
       calc_feed.*, 
       (p.valor_desconto / p.valor_venda_produto) * 100 AS porcentagem_desconto
FROM produto p 
LEFT JOIN ( 
...
AND (
    p.id_loja != (SELECT cod_loja FROM loja WHERE id_usu = $usuario_id) 
    OR NOT EXISTS (SELECT 1 FROM loja WHERE id_usu = $usuario_id)
)
ORDER BY porcentagem_desconto DESC";
        ...
        while ($info = mysqli_fetch_assoc($rs)) {
            ...
            echo "<p class='p-preco'>R$" . number_format($valorCompra, 2, ',', '.') . "</p>";
