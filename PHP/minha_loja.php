...
<body>
    ...
    <main>
        ...
        $sql_destaque = "SELECT p.*, p.foto_prod, l.nome AS nome_loja, l.banner, l.foto_loja, calc_feed.* 
                 ...
                 WHERE l.id_usu = $idLoja AND (p.qt_produto_estocado > 0 OR p.cod_produto IS NULL)
...
FROM loja l 
LEFT JOIN produto p ON p.id_loja = l.cod_loja
LEFT JOIN 
(SELECT f.id_produto AS prod, COUNT(*) AS qtd_ocorrencia, SUM(f.nota) AS soma, AVG(f.nota) AS media 
FROM feedback f
GROUP BY f.id_produto) AS calc_feed
ON p.cod_produto = calc_feed.prod
WHERE l.id_usu = $idLoja 
AND (p.qt_produto_estocado > 0 OR p.cod_produto IS NULL);";

$rs = mysqli_query($con, $sql);
$loja = mysqli_fetch_assoc($rs);


                    $imagemLoja = $loja['foto_loja'] ?? 'sem imagem.png';
                    $imagemBanner = $loja['banner'] ?? 'sem imagem.png';
                    $nome = $loja['nome_loja'] ?? 'sem nome'; 
